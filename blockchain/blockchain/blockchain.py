...
class Blockchain:
	...
	def __init__ (self, api_token, db=True):
		"""Inicializa la clase.
...
class Blockchain_L3 (Blockchain):
	...
	def get_stats_bids_asks (self, order_type):
		...
		if self.db:
			from blockchain.models import BidsAsks

		average_value = np.mean([ bidask["qty"]*bidask["px"] for bidask in self.data[order_type] ])
		...
		total_qty = sum([ bidask["qty"] for bidask in self.data[order_type] ])
		total_px = sum([ bidask["px"] for bidask in self.data[order_type] ])

		if self.db:
			for bidask in self.data[order_type]:
				r_bidsasks = BidsAsks.objects.filter(num=bidask["num"])
				if not r_bidsasks:
					bidsasks = BidsAsks()
					bidsasks.crypto = self.crypto
					bidsasks.realmoney = self.realmoney
					bidsasks.symbol = self.symbol
					bidsasks.order = order_type
					bidsasks.px = bidask["px"]
					bidsasks.qty = bidask["qty"]
					bidsasks.num = bidask["num"]
					logger.info("%s-%s: %s -> {'px': %f, 'qty': %f, 'num': %d, 'value': %f}" % (self.crypto, self.realmoney, order_type, bidask["px"], bidask["qty"], bidask["num"], bidask["px"]*bidask["qty"]))
					bidsasks.save()
...
Example:
  ...
  %prog -t e3b35a91-ec09-40b1-a2ef-f09f66e1f8d8 -c BTC -r GBP --stats general
	...
	if options.api_token and options.crypto and options.realmoney:
		bc = Blockchain_L3(api_token=options.api_token, db=False)
